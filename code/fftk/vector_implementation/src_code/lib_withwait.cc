...
int MPI_Alltoall(const void *sendbuf, int sendcount, MPI_Datatype sendtype,void *recvbuf, int recvcount, MPI_Datatype recvtype, MPI_Comm comm)
{
	...
	if(global_rank!=my_leader)
	{
		...
		MPI_Isend(sendbuf,s,MPI_BYTE,my_leader,global_rank,comm,&request);
		MPI_Wait(&request,MPI_STATUS_IGNORE);

                MPI_Irecv(recvbuf,s,MPI_BYTE,my_leader,global_rank,comm,&request);
		MPI_Wait(&request,MPI_STATUS_IGNORE);
	}
	else
	{
		...
		for(int i=global_rank-sameSocket_size+1;i<global_rank;i++)
		{
			MPI_Irecv(send_buf+(index*sendcount*datatype_size*sizeof(char)),1,newvtype,i,i,comm,&request[index]);
			index++;
		}
		...
		for(int i=global_rank-sameSocket_size+1;i<global_rank;i++)
		{
			MPI_Isend(recv_buf+(index*sameSocket_size*sendcount*datatype_size*sizeof(char)),1, newvtype2, i, i,comm,&request[index]);
			index++;
		}
