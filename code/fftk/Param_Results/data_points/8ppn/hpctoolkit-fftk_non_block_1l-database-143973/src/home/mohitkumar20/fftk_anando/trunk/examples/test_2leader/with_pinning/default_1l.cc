...
int  MPI_Init(int *argc,char **argv[])
{
	...
	if(global_rank==my_leader)
	{
		...
		MPI_Comm_size(leaderComm,&leader_size);
                ...
                request=(MPI_Request*)malloc(sizeof(MPI_Request)*(sameHost_size+1));
	}
	else
	{
...
{
        unsigned long long send_size=global_size*sameHost_size;
        send_size*=sendcount;
...
int MPI_Alltoall(const void *sendbuf, int sendcount, MPI_Datatype sendtype,void *recvbuf, int recvcount, MPI_Datatype recvtype, MPI_Comm comm)
{
	if(global_rank!=my_leader)
	{
		...
		unsigned long long  s=global_size*sendcount*datatype_size;
                ...
                MPI_Send(sendbuf,s,MPI_BYTE,sameHost_size-1,sameHost_rank,sameNodeComm);
                
                MPI_Recv(recvbuf,s,MPI_BYTE,sameHost_size-1,sameHost_rank,sameNodeComm,MPI_STATUS_IGNORE);
	...
	else
	{
		...
		{
			MPI_Irecv(send_buf+(index*sendcount*datatype_size*sizeof(char)),1,newvtype,i,i,sameNodeComm,&request[req_count++]);
			index++;
		}
		...
		for(int i=0;i<sameHost_size-1;i++)
		{
			MPI_Isend(recv_buf+(index*sameHost_size*sendcount*datatype_size*sizeof(char)),1, newvtype2, i, i,sameNodeComm,&request[req_count++]);
			index++;
		}
                MPI_Isend(recv_buf+(index*sameHost_size*sendcount*datatype_size*sizeof(char)),1, newvtype2,0,global_rank,MPI_COMM_SELF,&request[req_count++]);
